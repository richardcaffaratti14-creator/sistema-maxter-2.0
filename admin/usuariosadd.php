...
switch ($usuarios->CurrentAction) {
  ...
  case "C": // Copy an existing record
   if (!LoadRow()) { // Load record based on key
      ...
      Page_Terminate($usuarios->getReturnUrl()); // Clean up and return
    }
		break;
  case "A": // ' Add new record
    ...
    if (AddRow()) { // Add successful
      ...
      Page_Terminate($usuarios->KeyUrl($usuarios->getReturnUrl())); // Clean up and return
    } else {
...
function ew_ValidateForm(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm)) {
			if (!ew_OnError(elm, "Por favor ingrese el campo requerido - Nombre de usuario"))
				return false;
		}
...
<p><span class="phpmaker">Agregar TABLA: Usuarios<br><br><a href="<?php echo $usuarios->getReturnUrl() ?>">Volver atras</a></span></p>
<?php
...
?>
<form name="fusuariosadd" id="fusuariosadd" action="usuariosadd.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewTable">
  <tr class="ewTableRow">
    <td class="ewTableHeader">Nombre de usuario<span class='ewmsg'>&nbsp;*</span></td>
    <td<?php echo $usuarios->usuario->CellAttributes() ?>><span id="cb_x_usuario">
<input type="text" name="x_usuario" id="x_usuario" title="" size="30" maxlength="50" value="<?php echo $usuarios->usuario->EditValue ?>"<?php echo $usuarios->usuario->EditAttributes() ?>>
</span></td>
  </tr>
...
function AddRow() {
	...
	$bValidUser = FALSE;
	if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
		$bValidUser = $Security->IsValidUserID($usuarios->idUsuario->CurrentValue);
		if (!$bValidUser) {
			$_SESSION[EW_SESSION_MESSAGE] = "Desautorizado";
			return FALSE;
		}
	}
	...
	$bCheckKey = TRUE;
	$sFilter = $usuarios->SqlKeyFilter();
	if (trim(strval($usuarios->idUsuario->CurrentValue)) == "") {
		$bCheckKey = FALSE;
	} else {
		$sFilter = str_replace("@idUsuario@", ew_AdjustSql($usuarios->idUsuario->CurrentValue), $sFilter); // Replace key value
	}
	...
	if ($usuarios->usuario->CurrentValue <> "") { // Check field with unique index
		$sFilter = "(`usuario` = '" . ew_AdjustSql($usuarios->usuario->CurrentValue) . "')";
		$rsChk = $usuarios->LoadRs($sFilter);
		if ($rsChk && !$rsChk->EOF) {
