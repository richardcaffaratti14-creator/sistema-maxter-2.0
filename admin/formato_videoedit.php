...
if (@$_POST["a_edit"] <> "") {
	$formato_video->CurrentAction = $_POST["a_edit"]; // Get action code
	LoadFormValues(); // Get form values
} else {
	$formato_video->CurrentAction = "I"; // Default action is display
}

// Check if valid key
if ($formato_video->id->CurrentValue == "") Page_Terminate($formato_video->getReturnUrl()); // Invalid key, exit
switch ($formato_video->CurrentAction) {
	case "I": // Get a record to display
		...
		break;
	Case "U": // Update
		$formato_video->SendEmail = TRUE; // Send email on update success
...
// Render the record
$formato_video->RowType = EW_ROWTYPE_EDIT; // Render as edit
RenderRow();
?>
...
<p><span class="phpmaker">Editar TABLA: Formatos de videos<br><br><a href="<?php echo $formato_video->getReturnUrl() ?>">Volver atrás</a></span></p>
<?php
...
?>
<form name="fformato_videoedit" id="fformato_videoedit" action="formato_videoedit.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewTable">
	<input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($formato_video->id->CurrentValue) ?>">
	<tr class="ewTableRow">
...
function LoadFormValues() {
	...
	$formato_video->id->setFormValue($objForm->GetValue("x_id"));
...
function RestoreFormValues() {
	...
	$formato_video->id->CurrentValue = $formato_video->id->FormValue;
...
function EditRow() {
	...
	if ($formato_video->Sufijo->CurrentValue <> "") { // Check field with unique index
		$sFilterChk = "(`Sufijo` = '" . ew_AdjustSql($formato_video->Sufijo->CurrentValue) . "')";
		$sFilterChk .= " AND NOT (" . $sFilter . ")";
		$formato_video->CurrentFilter = $sFilterChk;
		$sSqlChk = $formato_video->SQL();
	...
	if ($rs->EOF) {
		...
		$bUpdateRow = $formato_video->Row_Updating($rsold, $rsnew);
		if ($bUpdateRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$EditRow = $conn->Execute($formato_video->UpdateSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
	...
	if ($EditRow) {
		$formato_video->Row_Updated($rsold, $rsnew);
	}
	$rs->Close();
	return $EditRow;
}
