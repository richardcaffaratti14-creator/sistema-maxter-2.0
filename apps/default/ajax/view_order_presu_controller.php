...
if ($pid != '' && $fid != '') {
	...
	if ($pb_mode) {
		PhotoBook::save();	//if it's new does not matters as save() handles the case
	}
    ...
    <? if (count($pedido) > 0) { ?>
		...
		foreach ($item['formats'] as $f) {
			...
			<? } ?>
	    	    ...
	    	    <span class="pedido-format"><?= $format->nombre ?><? if ($item['type'] != 'coreo') { ?> x <?= $f['qty'] ?> copias<? } ?></span>
	    	    <br /><br />
...
<script type="text/javascript">
	...
	$('#view-media').load('ajax/view_order_presu?pid=' + pid + '&fid=' + fid);
